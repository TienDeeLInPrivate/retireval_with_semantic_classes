...
filename_pubmed = "Bachelorarbeit_RAG/4.5_Re-Ranking_Semantische_Klassen/entailment/data/pubmed/reranked_result_pubmed_hybrid_10.pkl"
...
fig = px.line(top_k_df, x='Top K', y=top_k_df.columns[1:], markers=True,
              ...
              title="Vergleich der Top-K Genauigkeit: Re-Ranking (QNLI) vs Original")
fig.update_layout(
    ...
    title=dict(text="Vergleich der Top-K Genauigkeit: Re-Ranking (QNLI) vs Original", x=0.5, xanchor='center', font=dict(size=29)),
...
colors = ['#f7a6a6', '#d62728', '#7fb0e1', '#1f77b4']  # Color needs to be in exact order
